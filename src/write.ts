...
export const registerdFonts: Set<string> = new Set();

export function safeRegisterFont(fontPath: string) {
    if (!registerdFonts.has(fontPath)) {
        GlobalFonts.registerFromPath(fontPath);
        registerdFonts.add(fontPath);
    }
}
 ...
 */
export function getOrLoadFont(fontPath: string): Font {
...
export function fontPaths(weight: FontWeight): string[] {
    const fontsPath = FilePath.FONTS;
    const fontFamilies = [
        "Oswald",
        "NotoSansJP",
        "NotoSansKR",
    ...
    return fontFamilies.map(
        (family) => join(fontsPath, family, `${family}-${weight}.ttf`)
    );
}
...
export function checkGlyph(font: Font, glyph: string): boolean {
    try {
        return font.charToGlyph(glyph).index !== undefined;
    } catch { return false; }
}
...
export function renderSingleLine(
    ...
    for (const [txt, fontPath] of formatted) {
        ...
        const fontFamilyName = basename(dirname(fontPath));
...
export function textHeight(
    ...
    for (const line of lines) {
        let lineHeight = 0;
        const formatted = groupByFont(line, fontPaths);
